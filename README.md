...
> (`GMX_search_ui`), aus dem GitHub Pages sie ausliefert. Der Übertragungsweg
> und die manuellen Schritte stehen in
...
Und seit der Kopierfunktion: dass **nichts** vorausgewählt ist (geprüft an 200
Treffern), dass es keinen „Alle auswählen"-Schalter gibt, dass eine Auswahl an
...
die beiden Workflow-Dateien, Ergebnis-Branch. Kein Token, kein Schlüssel, keine
Adresse aus dem Postfach – auch nicht als Beispiel.
...
|---|---|---|
| Actions | Read and write | Recherche und Kopiervorgang starten, Laufstatus lesen |
...
ausschließlich vom Workflow mit dessen eigenem Token. Die Kopierfunktion
braucht **keine** zusätzlichen Rechte.

## Was diese Seite nicht tut

Sie verschiebt nichts, löscht nichts und ändert keine Flags. Kopiert wird nur,
was jemand einzeln angehakt und anschließend bestätigt hat – ein hoher
Punktwert wählt nichts aus. GMX-Zugangsdaten bekommt der Browser zu keinem
Zeitpunkt; sie liegen ausschließlich in den GitHub-Actions-Secrets.
